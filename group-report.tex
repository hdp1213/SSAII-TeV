...
\label{sec:gal-rot}
\paragraph{}
The Mopra data featured heavily in this report \parencite{Burton:2013} derives its usefulness from the effect that galactic rotation has on co-linear objects. Fig.~\ref{fig:gal-rot} shows how for a simplified model of the Milky Way with constant radial velocity, looking along a line of sight, in this particular instance one corresponding to a galactic latitude of \(l=337\), leads to a velocity gradient and a measurable Doppler shift. In order to find distances from the sun using the Mopra data, it is necessary to consider the component of velocity parallel to the line of sight. See Appendix~\ref{app:doppler} for the derivation. Taking the distance from the sun to the galactic centre (GC) as \(R_0\), and any other radial distance as \(R\), with distance from the sun as \(d\), the observed velocity of objects along a line of sight with galactic longitude \(l\) taking into account the sun's component of velocity around the galaxy is given by:
\begin{equation}
  v_{los}(d)=\frac{v(R)R_0\sin{l}}{\sqrt{d^2-2R_0d\cos{l}+R_0^2}}-v(R_0)\sin{l}
\end{equation}
This equation holds for a galaxy with variable rotation \(v(R)\) dependent on radial distance from the GC, and for the sake of simplicity in this report is taken to be a constant \(v(R)=\unit{220}{\metre}\) (reference thanks). The distance from the GC is also taken to be \(R_0=8\). Words words words, my friend!
...
\newpage
\appendix
\section{Galactic doppler shift}
\label{app:doppler}
\paragraph{}
